...
class specularitynetBase(BaseModel):
    ...
    def eval(self, data, epoch, savedir=None):
        ...
        with torch.no_grad():
            ...
            for _input, target, result, mask, detect, fn in zip(self.input, self.target_t, self.output_i, self.mask,
                                                                self.detect, self.data_name):
                ...
                cur_psnr=res['PSNR']
                #if savedir is not None and cur_psnr>max_psnr:
                #    max_psnr=cur_psnr
                #    os.makedirs(join(savedir), exist_ok=True)
                    
                    
                #if epoch % 10 == 0:
                if epoch == 5 or epoch == 20 or epoch == 40 or epoch==100 or epoch==90:
                    Image.fromarray(result).save(join(savedir, 'epoch{}_{}_result_{}.png'.format(epoch,fn, self.opt.name)))
                    Image.fromarray(target).save(join(savedir, 'epoch{}_{}_target.png'.format(epoch,fn)))
                    Image.fromarray(_input).save(join(savedir, 'epoch{}_{}_input.png'.format(epoch,fn)))
                    ...
                    '''if self.opt.suffix is not None:
                        Image.fromarray(result).save(
                            join(savedir, '{}_result_{}_{}.png'.format(fn, self.opt.name, self.opt.suffix)))
                    else:
                        Image.fromarray(result).save(join(savedir, 'epoch{}_{}_result_{}.png'.format(epoch,fn, self.opt.name)))
                    ...
                    if not detect.size == 0:
                        ...
                        if self.opt.suffix is not None:
                            Image.fromarray(detect).save(
                                join(savedir, '{}_detect_{}_{}.png'.format(fn, self.opt.name, self.opt.suffix)))
                            Image.fromarray(spec).convert('L').save(
                                join(savedir, '{}_specular_{}_{}.png'.format(fn, self.opt.name, self.opt.suffix)))
                        else:
                            Image.fromarray(detect).save(join(savedir, '{}_detect_{}.png'.format(fn, self.opt.name)))
                            Image.fromarray(spec).convert('L').save(
                                join(savedir, 'epoch{}_{}_specular_{}.png'.format(epoch, fn, self.opt.name)))'''
#==================================================================================================================================
            for key in res_overall:
    ...
    def test(self, data, savedir=None):
        ...
        with torch.no_grad():
            ...
            if self.data_name is not None and savedir is not None:
                os.makedirs(join(savedir), exist_ok=True)
                for _input, result, mask, detect, fn in zip(self.input, self.output_i, self.mask, self.detect,
                                                            self.data_name):
                    _input = tensor2im(_input)
                    result = tensor2im(result)
                    mask = tensor2im(mask)
                    ...
                    if not detect.size == 0:
                        ...
                        if self.opt.suffix is not None:
                            ...
                            Image.fromarray(detect).save(join(savedir, '{}_detect_{}.png'.format(fn, self.opt.name)))
                            Image.fromarray(spec).convert('L').save(
                                join(savedir, '{}_specular_{}.png'.format(fn, self.opt.name)))
...
class specularitynetModel(specularitynetBase):
    ...
    def initialize(self, opt):
        ...
        self.net_i.to(self.device)
        self.net_i = nn.DataParallel(self.net_i)
