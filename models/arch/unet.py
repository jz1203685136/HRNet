...
class ConvNormAct(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=True, norm=None,
                 num_groups=8, act='elu', negative_slope=0.1, inplace=True, reflect=True):
...
class GatedConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=True, norm=None,
                 num_groups=8, act='elu', negative_slope=0.1, inplace=True, \
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None, add_channels=None, kernels=None, gatedconv=True,
...
class RefinedNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, enhance='de', ppm=True, bilinear=True):
        ...
        self.down3 = nn.Sequential(
            DownBlock(d_channels[2], d_channels[3], kernels=[3, 3], gatedconv=False),
            DilationBlock(d_channels[3], d_channels[3], d_channels[3], dilations=[2, 4], gatedconv=False),
        )
        # four-th encoder
        self.resblock = ResBlock(d_channels[3], blocks=3, resscale=0.1, kernel_size=3, gatedconv=True, enhance=enhance,
                                 ppm=ppm, dilations=[2, 4])

        # first decoder
        self.up1 = UpBlock(u_channels[0], u_channels[1], u_channels[1], add_channels=d_channels[2], kernels=[5, 5],
                           gatedconv=False, bilinear=bilinear)
        # second decoder
        self.up2 = UpBlock(u_channels[1], u_channels[2], u_channels[2], add_channels=d_channels[1], kernels=[5, 5],
                           gatedconv=False, bilinear=bilinear)
        self.up3 = UpBlock(u_channels[2], u_channels[3], u_channels[3], add_channels=d_channels[0], kernels=[5, 5],
                           gatedconv=False, bilinear=bilinear)
        # coarse detection block
        self.coarse_detect = nn.Sequential(
            ConvNormAct(u_channels[3] + 3, 64, kernel_size=3, padding=1),
        ...
        self.coarse = nn.Sequential(
            ConvNormAct(u_channels[3] + 4, 64, kernel_size=3, padding=1),
            ConvNormAct(64, 64, kernel_size=3, padding=1),
            ConvNormAct(64, out_channels, kernel_size=3, padding=1, norm=None)
        )
        # refined detection block
        self.refined_detect = nn.Sequential(
            GatedConv(u_channels[3] + 7, 64, kernel_size=7, padding=3),
            ResBlock(64, blocks=3, resscale=1.0, kernel_size=3, gatedconv=True, enhance=enhance, ppm=ppm,
                     dilations=[2, 4]),
    ...
    def forward(self, x, iters=1):
        ...
        m = self.resblock(down3)
        up11 = self.up1(m, down2)
        up1 = self.RN_L1(up11)
        up22 = self.up2(up1, down1)
        up2 = self.RN_L2(up22)
        up3 = self.up3(up2, preconv)

        detect = self.coarse_detect(torch.cat([x, up3], dim=1))
        y = self.coarse(torch.cat([x, up3, detect], dim=1))
        coarse_list = []
        detect_list = [detect.squeeze(1)]
        for i in range(iters):
            coarse_list.append(y)
            detect = self.refined_detect(torch.cat([x, y, up3, detect], dim=1))
            y = self.refined(torch.cat([x, y, up3, detect], dim=1))
...
class DiscFeat(nn.Module):
    ...
    def forward(self, x,
                require_feat=False):  # if you require_feat, D also output the middle feature maps to calculate the loss
        feat = self.feat(x)
        y = self.disc(feat)
...
class SelfAware_Affine(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = torch.cat([avg_out*0.6, max_out*1.2], dim=1)
